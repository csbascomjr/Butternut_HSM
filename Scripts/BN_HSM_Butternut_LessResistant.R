...
#Subset location data to butternut families that are less resistant (susceptible) to BCD 
Location_Data_Suscept_JC <- Location_Data[Location_Data$species == "JC" 
                                            & Location_Data$bays_bcd_bv <= 0, c("bays_bcd_bv","fips")]
Location_Data_Suscept_JC$GEOID <- as.character(Location_Data_Suscept_JC$fips)
Location_Data_Suscept_JC$fips <- NULL
Location_Data_Suscept_JC <- na.omit(Location_Data_Suscept_JC)
length(unique(Location_Data_Suscept_JC$GEOID))
...
set.seed(6644)
df <- data.frame(
  GEOID = as.numeric(unique(Location_Data_Suscept_JC$GEOID)),
  ind = sample(2, length(unique(Location_Data_Suscept_JC$GEOID)), replace = TRUE, prob = c(0.7, 0.3))
)

df$GEOID <- sprintf("%05d", df$GEOID) #turns string to charcter


occurance_geoids <- df[df$ind == 1, c("GEOID")]
unavailable_GEOIDS <- c(VA_City_GEOIDS, as.character(Location_Data_Suscept_JC$GEOID))
...
unavailable_GEOIDS <- c(train.absence_GEOIDS, VA_City_GEOIDS, as.character(Location_Data_Suscept_JC$GEOID))
available_GEOIDS <- PRISM_Data[!PRISM_Data$GEOID %in% unavailable_GEOIDS, "GEOID"]
...
test.occurance <- test.occurance %>%
  left_join(Location_Data_Suscept_JC %>%
              group_by(GEOID) %>%
...
#Check to make sure the selected counties make sense
...
PRISM_Data[PRISM_Data$GEOID == "01033",]
predictor_table[predictor_table$GEOID == "01033",]
str(predictor_table)
...
spec.cor.topo_v2 <- predictor_table[,c(4:135)]
topo.cor_v2 <- cor(spec.cor.topo_v2) # correlation
corrplot.mixed(topo.cor_v2, lower = 'number', upper = 'square')
write.csv(topo.cor_v2, "./Results/20250501_HabitSuitMod/PRISM_Data_SusceptJC_Correlations_v1.csv", row.names=TRUE)
...
write.csv(predictor_table_final, "./Results/20250501_HabitSuitMod/PRISM_Data_GenoJCSuscept_PredictorTableFinal_v1.csv", row.names=FALSE)
#Bring up final predictor table, if wanted
predictor_table_final <- read.csv("./Results/20250501_HabitSuitMod/PRISM_Data_GenoJCSuscept_PredictorTableFinal_v1.csv", header=TRUE)
...
mod.gam.1 <- gam(as.factor(PA) ~ 
                   ...
                   s(mean_norm_soltotal_annual) +
                   s(mean_norm_soltrans_02) +
                   s(mean_norm_soltrans_08) +
                   s(mean_norm_vpdmin_01) +
...
mod.gam.2 <- gam(as.factor(PA) ~ 
                   ...
                   #s(mean_norm_soltotal_annual, k = 3) +
                   s(mean_norm_soltrans_02, k = 3) +
                   #s(mean_norm_soltrans_08, k = 3) +
                   #s(mean_norm_vpdmin_01, k = 3) +
...
summary(mod.gam.4)
gam.check(mod.gam.4)
AIC(mod.gam.1, mod.gam.2,
    mod.gam.3,mod.gam.4)


roc_obj <- roc(predictor_table_final$PA, predict(mod.gam.1, type = "response"))
auc(roc_obj)
...
mod4.fit <- 100*(1-mod.gam.4$deviance/mod.gam.4$null.deviance)
print(mod1.fit)
print(mod2.fit)
print(mod3.fit)
print(mod4.fit)

#Model 1, but may be over fitted
...
mod.rf.1 <- randomForest(as.factor(PA) ~ 
                           ...
                           mtry = 2, #sqrt(predictor_n) is a good place to start
                           #classwt = c("0" = 1, "1" = 5),
                           importance = TRUE,
                           proximity=TRUE) 
print(mod.rf.1)
...
mod.rf.2 <- randomForest(as.factor(PA) ~ 
                           ...
                           data = predictor_table_final, 
                           ntree = 2000, 
                           sampsize = c('0'=58,'1'=58),
                           mtry = 4, #sqrt(predictor_n) is a good place to start
                          #classwt = c("0" = 1, "1" = 5),
                           importance = TRUE,
                           proximity=TRUE) 
print(mod.rf.2)
...
test_predictor_table <- test_predictor_table[,c("GEOID","PA",
                                                ...
                                                'mean_norm_vpdmin_annual',
                                                'osc_mtu_per_ha')]
write.csv(test_predictor_table, "./Results/20250501_HabitSuitMod/PRISM_Data_GenoJCSuscept_TestDataPredictorTable.csv", row.names=FALSE)
...
test_predictor_table$mod.brt.predict <- 100*predict(mod.brt.1, newdata = test_predictor_table, type = "response",  n.trees = best_iter)

predictor_vars <- colnames(maxent_predictors_1)
test_data_subset <- test_predictor_table[, predictor_vars, drop = FALSE]
test_predictor_table$mod.me.predict <- 100*predict(mod.me.1, test_data_subset, type = "cloglog")
...
#Training Data
mod.rf.final.predict <- predict(mod.rf.final, type = "response") 
...
mod.me.final.acc <- presence.absence.accuracy(data.me, threshold = me.mod.cut$mod.me.predict,  st.dev = F)
tss <- mod.me.final.acc$sensitivity + mod.me.final.acc$specificity - 1
mod.me.final.acc <- cbind(mod.me.final.acc[1:7], tss)
pander::pander(mod.me.final.acc[c(1,4:5,7:8)])
...
PRISM_Data_4_rf <- PRISM_Data[,c("GEOID",
                                 "mean_norm_ppt_05",
                                 "mean_norm_ppt_07",
                                 'mean_norm_ppt_09',
                                 'mean_norm_ppt_10',
...
PRISM_Data_4_me <- PRISM_Data[complete.cases(PRISM_Data$osc_mtu_per_ha),c("GEOID",
                                                                          "mean_norm_ppt_05",
                                                                          "mean_norm_ppt_07",
                                                                          'mean_norm_ppt_09',
                                                                          'mean_norm_ppt_10',
...
nrow(me_predictors)
nrow(PRISM_Data_4_me)
nrow(predicted_prob_me)
...
write.csv(data, "./Results/20250501_HabitSuitMod/GenoJCSuscept_ModelsOutputContinuous.csv", row.names=FALSE)

#map it####
data <- read.csv("./Results/20250501_HabitSuitMod/GenoJCSuscept_ModelsOutputContinuous.csv", header=TRUE)
data$GEOID <- sprintf("%05d", as.integer(data$GEOID))
data_to_map <- merge(data,us_counties_shapefile_simpl, by = "GEOID")
data_to_map$model_concensus <- as.factor(data_to_map$model_concensus)

plot <- ggplot() +
  geom_sf(data = data_to_map, aes(geometry = geometry, fill = predicted_prob_glm), color = "NA", show.legend = FALSE) +
  theme_light() +
  labs(title = "HSM, GLM (60.87% Acc) \nwith FIA Data",
       subtitle = "J. cinerea") +
...
file_name1 <- paste("./Results/20250501_HabitSuitMod/pngs/",
                    "HSM_FIA_GLM",".png", sep = "")
ggsave(file_name1, plot, width = 4, height = 4, units = "in", dpi = 300)

plot <- ggplot() +
  geom_sf(data = data_to_map, aes(geometry = geometry, fill = predicted_prob_gam), color = "NA", show.legend = FALSE) +
  theme_light() +
  labs(title = "HSM, GAM (68.47% Acc) \nwith FIA Data",
       subtitle = " ") +
  scale_fill_gradient(low = "gold",
                      high = "firebrick",
                      limits = c(0,1))+
...
plot <- ggplot() +
  geom_sf(data = data_to_map, aes(geometry = geometry, fill = predicted_prob_rf), color = "NA", show.legend = FALSE) +
  theme_light() +
  labs(title = "HSM, RF (67.39% Acc) \nwith FIA Data",
       subtitle = " ") +
  scale_fill_gradient(low = "gold",
                      high = "firebrick",
                      limits = c(0,1))+
...
plot <- ggplot() +
  geom_sf(data = data_to_map, aes(geometry = geometry, fill = predicted_prob_me), color = "NA", show.legend = FALSE) +
  theme_light() +
  labs(title = "HSM, MaxEnt (61.96% Acc) \nwith FIA Data",
       subtitle = " ") +
  scale_fill_gradient(low = "gold",
                      high = "firebrick")+
  theme(
...
plot <- ggplot() +
  geom_sf(data = data_to_map, aes(geometry = geometry, fill = model_concensus), color = "grey", show.legend = TRUE) +
  theme_light() +
  labs(title = "Model Aggrement",
       subtitle = "J. cinerea") +
  scale_fill_manual(name = "# Of Models",
                    na.value = "grey",
                    values = c("1" = "#edf8e9", 
                               "2"= "#bae4b3",
                               "3" = "#74c476",
...
file_name1 <- paste("./Results/20250501_HabitSuitMod/pngs/",
                    "HSM_GenoJCSuscept_ModelConcensus",".png", sep = "")
ggsave(file_name1, plot, width = 7, height = 7, units = "in", dpi = 300)
