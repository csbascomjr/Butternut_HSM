...
Location_Data_JXC <- Location_Data[Location_Data$species == "JXC", c("family","fips")]
Location_Data_JXC$GEOID <- sprintf("%05d", Location_Data_JXC$fips)
length(unique(Location_Data_JXC$GEOID))
Location_Data_JXC$fips <- NULL
...
data_to_map_1 <- merge(Location_Data_JXC,us_counties_shapefile_simpl, by = "GEOID")

ggplot() +
  geom_sf(data = us_counties_shapefile_simpl, aes(geometry = geometry), fill = "grey", color = "NA", show.legend = TRUE) +
  geom_sf(data = data_to_map_1, aes(geometry = geometry), fill ="firebrick", alpha = 0.5, color = "NA", show.legend = TRUE) +
  theme_light() +
  #scale_fill_gradient(name="PctHyb",
  #                    low = "gold",
  #                    high = "firebrick")+
  theme(
...
df <- data.frame(
  GEOID = unique(Location_Data_JXC$GEOID),
  ind = sample(2, length(unique(Location_Data_JXC$GEOID)), replace = TRUE, prob = c(0.7, 0.3))
)
...
unavailable_GEOIDS <- c(VA_City_GEOIDS, as.character(Location_Data_JXC$GEOID))
...
unavailable_GEOIDS <- c(train.absence_GEOIDS, VA_City_GEOIDS, as.character(Location_Data_JXC$GEOID))
available_GEOIDS <- PRISM_Data[!PRISM_Data$GEOID %in% unavailable_GEOIDS, "GEOID"]
...
nrow(train.location[train.location$PA == "1",])
...
write.csv(topo.cor_v2, "./Results/20250501_HabitSuitMod/PRISM_Data_GenoJXCAll_Correlations_v1.csv", row.names=TRUE)
...
write.csv(predictor_table_final, "./Results/20250501_HabitSuitMod/PRISM_Data_GenoJXCAll_PredictorTableFinal_v1.csv", row.names=FALSE)
#Bring up final predictor table, if wanted
predictor_table_final <- read.csv("./Results/20250501_HabitSuitMod/PRISM_Data_GenoJXCAll_PredictorTableFinal_v1.csv", header=TRUE)
...
mod.gam.2 <- gam(as.factor(PA) ~ 
                   ...
                   s(mean_norm_vpdmax_07, k = 3) +
                   s(mean_norm_vpdmin_02, k = 3) +
...
mod.rf.1 <- randomForest(as.factor(PA) ~ 
                         ...
                         data = predictor_table_final, 
                         ntree = 5000,  
                         mtry = 5, #sqrt(predictor_n) is a good place to start
                         sampsize = c('0'=27,'1'=27),
                         importance = TRUE,
                         proximity=TRUE) 
print(mod.rf.1)
...
#Attempt to optimize via caret####
rf_formula <- as.factor(PA) ~ 
  mean_norm_ppt_05 +
  mean_norm_ppt_06 +
  mean_norm_ppt_09 +
  mean_norm_ppt_10 +
...
#caret cross-validations
train_control <- trainControl(
  method = "repeatedcv",
  number = 10,        # 10 folds
  repeats = 30,       # 30 repetitions = 300 total model fits
  verboseIter = FALSE,
...
training_pseudo_absences <- predictor_table_final[predictor_table_final$PA == "0", ]
nrow(training_pseudo_absences)
training_occurances <- predictor_table_final[predictor_table_final$PA == "1", ]
nrow(training_occurances)

balanced_pseudo_absences <- training_pseudo_absences[sample(nrow(training_pseudo_absences), 
                                                            size = nrow(training_occurances)), ]
balanced_data <- rbind(training_occurances, balanced_pseudo_absences)
...
roc_curve <- roc(balanced_data$PA, pred_probs)
auc(roc_curve) 

#####

#That didn't help. Tweek the normal way
mod.rf.2 <- randomForest(as.factor(PA) ~ 
                           mean_norm_vpdmax_07 +
                           mean_norm_solclear_06 +
                           mean_norm_ppt_annual +
                         ...
                         data = predictor_table_final, 
                         ntree = 5000,  
                         mtry = 3, #sqrt(predictor_n) is a good place to start
                         sampsize = c('0'=27,'1'=27),
                         importance = TRUE,
                         proximity=TRUE) 
print(mod.rf.2)
...
test_predictor_table <- test_predictor_table[,c("GEOID","PA",
                                                ...
                                                "mean_norm_vpdmin_annual",
                                                "osc_mtu_per_ha")]
write.csv(test_predictor_table, "./Results/20250501_HabitSuitMod/PRISM_Data_GenoJXCAll_TestDataPredictorTable.csv", row.names=FALSE)
...
"
GLM accuracy: 73.52941%
GAM accuracy: 79.41176%
 RF accuracy: %
BRT accuracy: %
 ME accuracy: %
"
...
PRISM_Data <- read.csv("./Results/20250415_Accessions_Climate/AllCounties_climate_OSC_elevation_v3.csv", header=TRUE)
PRISM_Data_4_glm <- PRISM_Data[,c("GEOID","mean_norm_ppt_05",
                                  "mean_norm_ppt_06",
                                  "mean_norm_ppt_09",
                                  "mean_norm_ppt_10",
                                  "mean_norm_ppt_annual",
...
write.csv(data, "./Results/20250501_HabitSuitMod/GenoJXCAll_ModelsOutputcontinuous.csv", row.names=FALSE)
